...
class ASipOfRedWine:
    def __init__(self,host='localhost',bindingip='',localport=5060,port=5060,
                 externalip=None,
                 username=None,crackmode=2,crackargs=None,realm=None,sessionpath=None,
                 selecttime=0.005,compact=False,reusenonce=False,extension=None,
                 maxlastrecvtime=5,domain=None):
...
if __name__ == '__main__':
    ...
    parser.add_option('--enabledefaults', '-D', action="store_true", dest="defaults",
                      default=True, help="""Scan for default / typical passwords such as
    ...
    if options.autogetip:
        ...
        tmpsocket.connect(("google.com",80))
        ...
        if lenres > 0:
            ...
            if (lenres < 400 and options.save is not None) or options.save is None:
                ...
                for k in sipvicious.resultpasswd.keys():
                    ...
                    data = "IP: " + host + " User: "+ k + " Pass: " +sipvicious.resultpasswd[k]
                    logging.info(data)
                    f=open('rsip.txt','a')
                    f.write( data +'\n')
                    f.close()
